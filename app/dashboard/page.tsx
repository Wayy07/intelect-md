...
import { formatDistanceToNow, format, isSameDay, subDays, subMonths } from "date-fns"
...
export default function AdminDashboard() {
  ...
  const formatDateRange = () => {
    ...
    return `${format(dateRange.from, 'dd MMM yyyy', { locale: ro })} - ${dateRange.to ? format(dateRange.to, 'dd MMM yyyy', { locale: ro }) : 'Prezent'}`
  }
  ...
  useEffect(() => {
    ...
    if (status === "authenticated" && session?.user?.role === "ADMIN" && dateRange.from) {
      ...
      <div className="container py-20">
        <div className="max-w-md mx-auto text-center">
          ...
          <p className="text-muted-foreground mb-6">
            Pagina pe care încercați să o accesați nu există sau nu aveți permisiunile necesare.
          </p>
  ...
  return (
    <div className="container py-6 md:py-10">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-2xl md:text-3xl font-bold mb-4 md:mb-6">Panou de administrare</h1>
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <div className="mb-6 flex flex-col md:flex-row md:items-center gap-3 bg-background rounded-lg p-4 shadow-sm border">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {DATE_RANGES.map((range: { label: string; value: string }) => (
                          <Button
                            ...
                            size="sm"
                            onClick={() => {
                              handlePresetChange(range.value);
                    ...
                    <div className="border rounded-md overflow-hidden">
                      <CalendarComponent
                        ...
                        onSelect={(range: DayPickerDateRange | undefined) => {
                          if (range?.from) {
                            setDateRange(range);
                            // Don't close popover automatically to allow adjustments
                            if (range?.to) {
              ...
              <Button
                ...
                onClick={() => {
                  const now = new Date();
                  const from = dateRange.from ? new Date(dateRange.from) : subDays(now, 30);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
              <Card>
                <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
                  <CardTitle className="text-base font-medium">Venituri totale</CardTitle>
                  <DollarSign className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold">
                    {dashboardData ? formatCurrency(dashboardData.financialStats.totalRevenue) : '0 MDL'}
                  </div>
                  <p className="text-xs text-muted-foreground">
                    {dashboardData && dashboardData.financialStats.totalRevenue > 0 ?
                      `+${Math.round((dashboardData.financialStats.monthlyRevenue / dashboardData.financialStats.totalRevenue) * 100)}% față de luna trecută` :
              ...
              <Card>
                <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
                  <CardTitle className="text-base font-medium">Comenzi</CardTitle>
                ...
                <CardContent>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {dashboardData?.orderStats.pending || 0} comenzi în așteptare
                  </p>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
                  <CardTitle className="text-base font-medium">Clienți</CardTitle>
              ...
              <Card>
                <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
                  <CardTitle className="text-base font-medium">Venituri luna curentă</CardTitle>
                  <TrendingUp className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold">
                    {dashboardData ? formatCurrency(dashboardData.financialStats.monthlyRevenue) : '0 MDL'}
                  </div>
                  <p className="text-xs text-muted-foreground">
                    {dashboardData?.orderStats.completed || 0} comenzi finalizate
                  </p>
                </CardContent>
              </Card>
            </div>
                  ...
                  <div>
                    ...
                    <CardDescription>
                      {dateRange.from && dateRange.to ?
                        `Vânzări între ${format(dateRange.from, 'dd MMM yyyy', { locale: ro })} și ${format(dateRange.to, 'dd MMM yyyy', { locale: ro })}` :
                        ...
                        <XAxis
                          ...
                          tickFormatter={(str) => {
                            try {
                              ...
                              return date.toLocaleDateString('ro-MD', { month: 'short', day: '2-digit' });
                            } catch (e) {
                              return str;
                            }
                          }}
                        ...
                        <Tooltip
                          labelFormatter={(value) => {
                            try {
                              ...
                              return `Vânzări: ${format(new Date(value), 'dd MMM yyyy', { locale: ro })}`;
                            } catch (error) {
                              return `Vânzări: ${value}`;
                            }
                          }}
                          formatter={(value) => [`${formatCurrency(Number(value))}`, 'Sumă']}
                        />
                        <Area
                          type="monotone"
                          dataKey="amount"
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        {dashboardData &&
                         ...
                         dashboardData.chartData.orderStatusDistribution.length > 0 ? (
                          <Pie
                            ...
                            nameKey="name"
                            label={({ name, percent }) =>
                              `${STATUS_TRANSLATIONS[name] || name}: ${(percent * 100).toFixed(0)}%`
                            }
                            paddingAngle={2}
                          >
                            {dashboardData.chartData.orderStatusDistribution.map((entry, index) => (
                              <Cell
                                key={`cell-${index}`}
                        ...
                        {dashboardData &&
                         ...
                         dashboardData.chartData.orderStatusDistribution.length > 0 && (
                          <>
                            <Tooltip
                              formatter={(value) => [`${value} comenzi`, 'Cantitate']}
                              labelFormatter={(name) => STATUS_TRANSLATIONS[name] || name}
                              contentStyle={{
                            ...
                            />
                            <Legend
                              formatter={(value) => STATUS_TRANSLATIONS[value] || value}
                              layout="horizontal"
